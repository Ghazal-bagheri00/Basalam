document.getElementById("registerForm").addEventListener("submit", async (e) => {
  ...
  // گرفتن مقدار نقش کاربری (مثلاً user یا employer)
  const role = document.querySelector('input[name="role"]:checked')?.value || "user";

  // اعتبارسنجی پر بودن همه فیلدها
  if (!username || !password || !first_name || !last_name || !province) {
    alert("لطفاً همه‌ی فیلدها را کامل وارد کنید.");
    return;
  }

  // اعتبارسنجی شماره تلفن (فرمت 09xxxxxxxxx)
  ...
  // اعتبارسنجی رمز عبور: حداقل 8 کاراکتر، حروف بزرگ، کوچک و عدد
  ...
  // تعیین نقش بر اساس role
  const is_admin = false; // فرض بر این است که ادمین از طریق بک‌اند تنظیم شود

...
class UserJobDB(Base):
    ...
    city_id = Column(Integer, ForeignKey("cities.id"), nullable=False)  # اضافه کردن ForeignKey

    user = relationship("UserDB", back_populates="jobs")
    job = relationship("JobDB", back_populates="user_jobs")
    city = relationship("CityDB")  # می‌توانید back_populates هم اضافه کنید اگر نیاز بود


class MessageDB(Base):  # تغییر نام کلاس برای یکپارچگی
